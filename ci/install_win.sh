...
then
    ...
    $VCPKG_ROOT = "$env:VCPKG_INSTALLATION_ROOT/installed/x64-windows"

    # Tell the compiler and linker where to look
    $env:INCLUDE = "$VCPKG_ROOT/include;$env:INCLUDE"
    $env:LIB = "$VCPKG_ROOT/lib;$env:LIB"
    $env:PATH = "$VCPKG_ROOT/bin;$env:PATH" # For DLLs at runtime
fi
